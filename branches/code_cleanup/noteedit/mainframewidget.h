...
/*----------------------------- update of buttons due to selection ------------------*/
		...
		void updateInterface(property_type status, int length);
...
/*------------------------------- selection ---------------------------------------*/
		...
		int xori_;
		QRect   selRect_;
		int checkAllStaffsForNoteInsertion(const int line, const QPoint p, property_type *status, bool *playable, bool *delete_elem, bool *insert_new_note);
