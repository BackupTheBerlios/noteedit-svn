...
void NRest::calculateDimensionsAndPixmaps() {
	...
	switch (length_) {
	      case MULTIREST:
			...
			break;
	      case WHOLE_LENGTH: blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::rfullMagPixmap_ : NResource::rfullPixmap_ ;
			redPixmap_   = NResource::rfullRedPixmap_;
			greyPixmap_   = NResource::rfullGreyPixmap_;
			lineoffs = -2;
			pointyoffs = -2;
			break;
	      case HALF_LENGTH: blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::rhalfMagPixmap_ : NResource::rhalfPixmap_;
			...
			yoffs = 5;
			break;
	      case QUARTER_LENGTH: blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::rquarterMagPixmap_ : NResource::rquarterPixmap_;
			redPixmap_   = NResource::rquarterRedPixmap_;
			greyPixmap_   = NResource::rquarterGreyPixmap_;
			lineoffs = -1;
			pointyoffs = 30;
			break;
	      case NOTE8_LENGTH: blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::r8MagPixmap_ : NResource::r8Pixmap_;
			redPixmap_   = NResource::r8RedPixmap_;
			greyPixmap_   = NResource::r8GreyPixmap_;
			lineoffs = -3;
			pointyoffs = 30;
			break;
	      case NOTE16_LENGTH : blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::r16MagPixmap_ : NResource::r16Pixmap_;
			redPixmap_   = NResource::r16RedPixmap_;
			greyPixmap_   = NResource::r16GreyPixmap_;
			lineoffs = -2;
			pointyoffs = 40;
			break;
	      case NOTE32_LENGTH :blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::r32MagPixmap_ : NResource::r32Pixmap_;
			redPixmap_   = NResource::r32RedPixmap_;
			greyPixmap_   = NResource::r32GreyPixmap_;
			lineoffs = -1;
			pointyoffs = 50;
			break;
		case NOTE64_LENGTH : blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::r64MagPixmap_ : NResource::r64Pixmap_;
			...
			break;
		case NOTE128_LENGTH  :
			blackPixmap_ = (properties_ & PROP_HIDDEN ) ? NResource::r128MagPixmap_ : NResource::r128Pixmap_;
...
void NRest::draw(int flags) {
	if ((properties_ & PROP_HIDDEN) && (flags & DRAW_NO_HIDDEN_REST)) return;
	...
	if (properties_ & DOT_MASK) { //  draw the dot for dotted rests
		main_props_->tp->setPen(actual_ ? NResource::redPen_ : NResource::blackPen_);
	...
	if (properties_ & PROP_FERMT) { //  fermate draw
