...
def getCompiler(env,sys):
	...
	if match:
		...
		env.pprint('GREY', 'could not parse '+platform)
	return False
...
def generate(env):
	...
	if 'install' in sys.argv:
		...
		if dd:
			env['DESTDIR']=dd
			env.pprint('PURPLE','** Enabling DESTDIR for the project ** ',env['DESTDIR'])
	...
	opts.AddOptions(
		...
		('WITH_FADEIN', 'switch on fade in/out support for (de)crescendo (default: yes)' ),
		('WITH_PRINTING','switch on printing support (default: yes)'),
	...
	if env['_USECOLORS_']:
		...
		l='%slinking%s $TARGET' % (env['BKS_COLORS']['GREY'], env['BKS_COLORS']['NORMAL'])
		#env['CCCOMSTR']    =c
		#env['SHCCCOMSTR']  =c
		#env['CXXCOMSTR']   =c
		#env['SHCXXCOMSTR'] =c
	...
	def symlink_str(target, source, env):
		...
		normal=''
		if env['_USECOLORS_']:
			purple=env['BKS_COLORS']['PURPLE']
			normal=env['BKS_COLORS']['NORMAL']
		return "%ssymlinking%s %s (-> %s)" % (purple, normal, target[0].path, str(source[0].name) )
	...
	if (not env.has_key('WITH_FADEIN') or ( env['WITH_FADEIN'] != "no" and env['WITH_FADEIN'] != "n") ):
		env.AppendUnique( CXXFLAGS = ['-DWITH_FADE_IN=1'])
		env.pprint('GREY','Enabling fade in support.')
	else:
		env.pprint('GREY','Disabling fade in support.')
			
	if (not env.has_key('WITH_PRINTING') or ( env['WITH_PRINTING'] != "no" and env['WITH_PRINTING'] != "n") ):
		env.AppendUnique( CXXFLAGS = ['-DWITH_DIRECT_PRINTING=1'])
