...
def generate(env):
	...
	if 'install' in sys.argv:
		...
		if dd:
			env['DESTDIR']=dd
			env.pprint('VIOLET','** Enabling DESTDIR for the project ** ',env['DESTDIR'])
	...
	if not env['HELP'] and (env['_CONFIGURE_'] or not env.has_key('ISCONFIGURED')):
		...
		if env['ARGS'].get('debug', None):
			...
			env.pprint('VIOLET','** Enabling debug for the project **')
		...
		if env['ARGS'].has_key('prefix'):
			...
			env.pprint('VIOLET','** installation prefix for the project set to:',env['PREFIX'])
