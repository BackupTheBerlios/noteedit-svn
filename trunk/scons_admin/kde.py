...
def detect_kde(env):
	...
	if len(kde_config):
		p('GREY', 'kde-config was found as '+kde_config)
	else:
	...
	if qtdir:
		...
		if m:
			qtdir = m.group(1)
			p('GREY',"qt was found as "+m.group(1))
		else:
	...
	if os.path.isfile(uic):
		p('BLUE',"uic was found as "+uic)
	else:
		uic = os.popen("which uic 2>/dev/null").read().strip()
		if len(uic):
			p('GREY',"uic was found as "+uic)
		else:
			uic = os.popen("which uic 2>/dev/null").read().strip()
			if len(uic):
				p('GREY',"uic was found as "+uic)
	...
	if os.path.isfile(moc):
		p('BLUE',"moc was found as "+moc)
	else:
		moc = os.popen("which moc 2>/dev/null").read().strip()
		if len(moc):
			p('GREY',"moc was found as "+moc)
	...
	if qtincludes and os.path.isfile(qtincludes + "/qlayout.h"):
		...
		p('BLUE',"ok "+qtincludes)
	else:
		if os.path.isfile(qtdir + "/include/qlayout.h"):
			# Automatic detection
			p('BLUE',"ok "+qtdir+"/include/")
			qtincludes = qtdir + "/include/"
		elif os.path.isfile("/usr/include/qt3/qlayout.h"):
			# Debian probably
			p('GREY','the qt headers were found in /usr/include/qt3/')
	...
	if os.path.isfile(kdeincludes + "/klineedit.h"):
		p('BLUE',"ok "+kdeincludes)
	else:
		if os.path.isfile(kdeprefix+"/include/kde/klineedit.h"):
			# Debian, Fedora probably
			p('GREY',"the kde headers were found in %s/include/kde/"%kdeprefix)
