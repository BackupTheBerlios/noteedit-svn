...
def generate(env):
	...
	def KDEprogram(lenv, target, source, 
			includes='', localshlibs='', localstaticlibs='', globallibs='', globalcxxflags=''):
		""" Makes a kde program 
		...
		if len(localstaticlibs)>0:
			lsst=lenv.make_list(localstaticlibs)
			lenv.link_local_staticlib(lsst)
			lenv.Depends( program_list, lsst )
